...
class PredictLabel(Resource):
    ...
    def post(self,query):
        print(query)
        if self.find_by_name(query) is None:
            return {'message': "An item with query '{}' already exists.".format(query)}
        else:
            X=[]
            X.append(query)
            X_arr=np.asarray(X)
...
api.add_resource(PredictLabel, '/predictLabel/<string:query>')
